...
- **Pattern, statistiche della settimana**: sonno per notte (22–7) con la media, latte al giorno con le poppate al seno, e "Quando mangia", il ritmo delle pappe e dei pianti giorno per giorno sulle 24 ore. Tre grafici leggibili a colpo d'occhio, solo numeri e mai giudizi.
...
- **Cronometro** in Home per la pappa al biberon, l'allattamento al seno (con cambio lato e tempi per lato) e il tiralatte: riparte dal tempo giusto anche dopo aver chiuso l'app; a fine poppata la voce si salva da sola con durata e lato, il tiralatte con i ml a tap. In Pattern, "Allattamento": minuti al giorno, media per poppata, ripartizione dei lati, ml tirati negli ultimi 7 giorni.
...
- Estensioni: una funzione nuova vive in `js/<nome>.js` e si registra su `window.AlanExt` (percorsi a tap, blocchi Home, tab, riquadri in Pattern/Salute/Altro, descrizioni nel diario, hook `change`; l'API è in `js/app.js`, sezione estensioni) senza toccare `app.js`. Si carica con un `<script>` in `index.html` dopo `app.js`, ha il suo blocco CSS `/* == nome == */` in `css/app.css`, il suo paragrafo in `SPEC.md` §9 e la sua suite `tests/<nome>.test.js`, che la carica con `boot({ext:['<nome>']})`.
