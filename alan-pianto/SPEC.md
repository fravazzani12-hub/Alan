...
| cry | dur (s), label (fame/sonno/cambio/aria/contatto/solo/null), bins {f,a,h}, ctx (snapshot), feat {vec[12], meanF0, sdF0, meanRms, bursts10, meanBurst, meanPause, voiced, cent, durS}, audio (bool locale), mime | audio in IndexedDB, chiave = id |
...
## 6. Roadmap
1. Audio condiviso: bucket privato Supabase Storage `cries/<family>/<id>`, upload dopo il salvataggio, download on-demand per il riascolto.
