...
## Struttura
- `index.html` markup; `css/app.css` stile (token in :root, dark via prefers-color-scheme); `js/app.js` UI + motore + audio + diagnostica + aggiornamenti; `js/sync.js` sync; `sw.js` cache shell + aggiornamento con banner (a ogni release alza `VERSION` in sw.js, è l'unico numero da toccare: senza bump i telefoni restano sulla versione vecchia; niente `?v=` sugli asset); `supabase/schema.sql` schema (rieseguibile).
