...
(async()=>{
  ...
  A.flow('diaper');assert.ok(/Pipì\?/.test(txt('#screenInner'))&&/>Sì</.test(app.els['#screenInner']._h)&&!/Poca|Tanta/.test(txt('#screenInner')),'solo sì/no');A.pick('pipi','si');assert.ok(/Cacca\?/.test(txt('#screenInner')));A.finish('no');assert.ok(/Cambio pipì sì, cacca no/.test(txt('#diary')));
  S.events.push({id:'dold',k:'diaper',t:Date.now()-2*60000,who:'Ilaria',pipi:'poca',cacca:'tanta'});T('renderDiary()');assert.ok(/Cambio pipì sì, cacca sì/.test(txt('#diary')),'valori vecchi poca/tanta letti come sì');
