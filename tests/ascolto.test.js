...
(async()=>{
  ...
  const row=String(app.els['#home-ascolto']._h);
  assert.ok(/Ascolto del pianto/.test(row)&&/>Spegni</.test(row));
  assert.ok(/>Scuro</.test(row)&&/AlanExt.ascolto.dark\(\)/.test(row),'lo schermo scuro si accende da Oggi: '+row.slice(0,200));
  AS.state().on=false;
