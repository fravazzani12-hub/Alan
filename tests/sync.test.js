...
const ch={state:'closed',_on:{},on(type,f,cb){ch._on[type==='postgres_changes'?f.table:type]=cb;return ch},subscribe(cb){ch._cb=cb;setTimeout(()=>{ch.state='joined';cb('SUBSCRIBED')},5);return ch},presenceState(){return {u1:[{who:'Fabio'}],u2:[{who:'Ilaria',at:1}]}},track(m){calls.push(['track',m]);return Promise.resolve('ok')}};
const bucket={};
global.window={ALAN_CONFIG:{SUPABASE_URL:'https://x.supabase.co',SUPABASE_ANON_KEY:'k'},addEventListener(){},supabase:{createClient(url,key,opts){calls.push(['client',url,opts]);return {from:table,
  auth:{getSession:()=>Promise.resolve({data:{session:{user:{id:'u1',email:'f@x'}}}}),onAuthStateChange(cb){setTimeout(()=>cb('INITIAL_SESSION',{user:{id:'u1',email:'f@x'}}),1)},signInWithPassword:a=>Promise.resolve({data:{},error:a.password==='bad'?{message:'Invalid login credentials'}:null}),signOut:()=>Promise.resolve({})},
...
(async()=>{
  ...
  const row=Sync._eventToRow({id:'x',t:5,k:'cry',who:'Fabio',label:null,audio:true,cloud:'x.m4a',mime:'audio/mp4',_updated:'2026-09-17T11:00:00.000Z',_deleted:true},false);
  assert.deepStrictEqual(row,{id:'x',family_id:'F',t:5,k:'cry',who:'Fabio',data:{label:null,cloud:'x.m4a',mime:'audio/mp4'},deleted:false,updated_at:'2026-09-17T11:00:00.000Z'});
  ...
  ch._on.presence();assert.deepStrictEqual(Sync.status().others,[{who:'Ilaria',at:1}]);
  ...
  const up=await Sync.uploadAudio('c1.m4a','BYTES','audio/mp4');assert.strictEqual(up.error,null);assert.ok(bucket['F/c1.m4a']);assert.strictEqual(bucket['F/c1.m4a'].o.contentType,'audio/mp4');assert.strictEqual(bucket['F/c1.m4a'].o.upsert,true);
  const dl=await Sync.downloadAudio('c1.m4a');assert.strictEqual(dl.data.type,'audio/mp4');
  await Sync.removeAudio('c1.m4a');assert.strictEqual(bucket['F/c1.m4a'],undefined);
  const miss=await Sync.downloadAudio('c1.m4a');assert.ok(miss.error);assert.strictEqual(Sync.status().lastError.where,'audio');
