...
/* ---------- recorder ---------- */
function startRec(){
  rec={t:Date.now(),frames:[],chunks:[],blob:null,mime:'',status:'starting',err:null,stream:null,ctx:null,an:null,mr:null,timer:null,stopped:false};
...
A.stopRec=function(){
  ...
  finishRec().then(function(){
    ...
    if(rec&&rec.blob)idbPut('audio',e.id,rec.blob).then(function(ok){if(!ok){e.audio=false;save();}});
...
function renderCries(){
  ...
  if(!cries.length){h+='<div class="list"><div class="empty">Nessun pianto registrato. Alla prossima crisi tocca "Piange".</div></div>';}
    ...
    cries.slice(0,60).forEach(function(e){
      ...
      h+='<div class="row" onclick="A.cryDetail(\''+e.id+'\')"><div class="time">'+(dayKey(e.t)===dayKey(Date.now())?'':'<small>'+dayLabel(e.t)+'</small><br>')+fmtTime(e.t)+'</div><div class="what">'+tag+' <span class="detail">'+det+'</span></div>'+(e.audio?'<button class="play" aria-label="Ascolta" onclick="event.stopPropagation();A.play(\''+e.id+'\')">▶</button>':'<span></span>')+'</div>';
    });
    h+='</div>';
  }
  el.innerHTML=h;
...
A.play=function(id){
  ...
  idbGet('audio',id).then(function(blob){
    if(!blob){toast('Audio non presente su questo telefono');return;}
    var url=URL.createObjectURL(blob),a=new Audio(url);playing=a;a.onended=function(){URL.revokeObjectURL(url);};a.play().catch(function(){toast('Il browser non riproduce questo formato');});
  });
};
A.cryDetail=function(id){
  ...
  if(e.audio)h+='<button class="btn ghost" onclick="A.play(\''+id+'\')">▶ Ascolta</button><div class="spacer"></div>';
