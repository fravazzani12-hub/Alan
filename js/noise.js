/* Estensione "noise": rumore bianco per la nanna, generato nel telefono (nessun file, nessun YouTube, funziona offline).
   ...
   i controlli sulla schermata di blocco; timer di spegnimento controllato su 'timeupdate'. Impostazioni in localStorage
...
var KEY='alan.noise',FS=44100,LOG_N=21,BASE_DB=-15;
...
var buf={type:null,L:null,R:null},audio=null,url=null,playing=false,endAt=null,tick=null,prepT=null;

function ensure(type){if(buf.type===type&&buf.L)return buf;buf={type:type,L:synth(type),R:synth(type)};return buf;}
function player(){
...
function load(){
  var b=ensure(st.type),a=player(),data=wav(b.L,b.R,gainDb(st.vol)),u=null;
...
/* avvio: tutto sincrono nel tap (iOS pretende play() dentro il gesto) */
function start(){
  var a=load();playing=true;
  if(st.timer){endAt=Date.now()+st.timer*60e3;tick=setInterval(check,15000);}else clearTimer();
  if(typeof a.play==='function'){try{var p=a.play();if(p&&p.then)p.then(null,function(){playing=false;clearTimer();X.refresh();API.toast('Il telefono non ha fatto partire il suono: riprova con un tap');});}catch(e){playing=false;}}
  mediaSession();X.refresh();
}
function stop(){
...
/* riprova subito con le nuove impostazioni se sta suonando */
function setType(t){if(typeOf(t)[0]!==t)return;st.type=t;save();if(playing)start();else X.refresh();}
function setVol(v){v=+v;if(!(v>=1&&v<=5))return;st.vol=v;save();if(playing)start();else X.refresh();}
function setTimer(m){m=+m;if(!TIMERS.some(function(t){return t[0]===m;}))return;st.timer=m;save();if(playing){if(m){endAt=Date.now()+m*60e3;if(!tick)tick=setInterval(check,15000);}else clearTimer();}X.refresh();}
/* prepara il suono salvato qualche secondo dopo l'avvio, così il primo tap è immediato */
function prewarm(ms){if(prepT)clearTimeout(prepT);prepT=setTimeout(function(){try{ensure(st.type);}catch(e){}},ms==null?4000:ms);}
...
function status(){if(!playing)return '';return endAt?'In riproduzione · si spegne alle '+API.fmtTime(endAt):'In riproduzione';}
...
function render(){
  ...
  h+='<h2>Suono</h2><div class="nz-types">'+TYPES.map(function(t){return '<button class="nz-type'+(t[0]===st.type?' on':'')+'" onclick="AlanExt.noise.setType(\''+t[0]+'\')"><b>'+t[1]+'</b><small>'+t[2]+'</small></button>';}).join('')+'</div>';
  h+='<h2>Volume</h2><p class="hint nz-hint">4 = come il video, a parità di volume del telefono.</p>'+chips([1,2,3,4,5].map(function(v){return [v,String(v)];}),st.vol,'setVol');
...
X.noise={KEY:KEY,FS:FS,LOG_N:LOG_N,BASE_DB:BASE_DB,CURVE_VIDEO:CURVE_VIDEO,TYPES:TYPES,VOL_DB:VOL_DB,TIMERS:TIMERS,
  ...
  open:open,summary:summary,status:status,render:render,prewarm:prewarm,player:function(){return audio;},buffers:function(){return buf;},ensure:ensure};
prewarm();
X.refresh();
})();
